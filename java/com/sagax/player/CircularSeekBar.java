...
public class CircularSeekBar extends View {
	...
	private static final int DEFAULT_CIRCLE_PROGRESS_COLOR = Color.argb(200, 255, 215, 0);
	private static final int DEFAULT_POINTER_COLOR = Color.argb(100, 255, 215, 0);
	private static final int DEFAULT_POINTER_HALO_COLOR = Color.argb(135, 74, 138, 255);
	private static final int DEFAULT_DRAG_COLOR = Color.argb(255,  205, 135, 0);
	...
	private void initPaints() {
		...
		mCircleProgressGlowPaint.setStyle(Paint.Style.STROKE);
	...
	protected void onDraw(Canvas canvas) {
		...
		Path path = new Path();	
		float angle = mEndAngle - mStartAngle + 360 - mProgressDegrees;
		if(angle < 20){
			angle = 20;
		}
		path.addArc(mCircleRectF,  mStartAngle + mProgressDegrees,angle);
		
		canvas.drawTextOnPath(time, path, 0, -15, mCirclePaint);
		canvas.drawArc(mCircleRectF, mStartAngle + mProgressDegrees, mEndAngle - mStartAngle + 360 - mProgressDegrees,true,mPointerPaint);
		
		if(mUserIsMovingPointer){
			mCircleProgressGlowPaint.setColor(DEFAULT_DRAG_COLOR);
		}else{
			mCircleProgressGlowPaint.setColor(mCircleProgressColor);
		}
		
		canvas.drawLine(0, 0, mPointerPositionXY[0], mPointerPositionXY[1], mCircleProgressGlowPaint);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mCircleHeight = (float)height / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 1.5f) - 18;
		mCircleWidth = (float)width / 2f - mCircleStrokeWidth - mPointerRadius - (mPointerHaloBorderWidth * 1.5f) - 18;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		float innerRadius = Math.min(mCircleHeight, mCircleWidth) - 3*additionalRadius; // Min inner radius of the circle, including the minimumTouchTarget or wheel width
		...
		case MotionEvent.ACTION_DOWN:
			...
			else if(Math.abs(cwDistanceFromStart - mProgressDegrees) > 20){
				mUserIsMovingPointer = false;
				return false;				
			}
