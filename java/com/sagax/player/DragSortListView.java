...
public class DragSortListView extends ListView {
	...
	public static final int HEADER_VISIBLE = 0 , HEADER_GONE = 1;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
		...
		//mHandler.obtainMessage(HEADER_VISIBLE).sendToTarget();	

		/*
		if( ev.getAction() == MotionEvent.ACTION_DOWN ){
			//System.out.println("\n\n\nDDDDDDDOOOOOOOOWWWWWNN\n\n\n");
			mScrollPosition = ev.getY();
		}
		if( ev.getAction() == MotionEvent.ACTION_MOVE && mHandler != null ){
			float dy = mScrollPosition - ev.getY();
			System.out.println( "-------- dy:"+dy+" position:"+mScrollPosition+"-------");	
			
			// if it's down scrolling and the header is not visible
			// make a notification to notice the header.
			if( dy < -20 ){
				mHeaderVisible = true;		
				mHandler.obtainMessage(HEADER_VISIBLE).sendToTarget();	
			}else if( dy > 20 ){
				mHeaderVisible = false;
				mHandler.obtainMessage(HEADER_GONE).sendToTarget();	
			}
